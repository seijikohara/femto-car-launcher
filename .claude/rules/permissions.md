...
| `INTERNET` | MET Norway weather API (`api.met.no`), OpenFreeMap vector map tile fetch (MapLibre), optional self-hosted Nominatim reverse geocoding (the default geocoder is on-device and needs no network), the optional live-map terrain layer (Mapterhorn DEM tiles), the optional BYO Mapbox (`api.mapbox.com`) and Google Maps (`maps.googleapis.com`) map backends — network-active only once the user supplies their own token/key — and the on-demand Google Fonts catalog + TTF download (`fonts.google.com` / `fonts.gstatic.com`, no API key, no Play Services). |
