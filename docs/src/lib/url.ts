...
// Everything at or below U+0020 — the C0 range plus the space itself. Module
// scope: it captures nothing from trimUrl, so there is no reason to recreate
// it on every call.
...
function trimUrl(path: string): string {
    let start = 0;
    let end = path.length;
    while (start < end && isC0OrSpace(path.charCodeAt(start))) start++;
    while (end > start && isC0OrSpace(path.charCodeAt(end - 1))) end--;
