...
// Paint map of one layer by id; module-scoped so it is not re-created per
// test (unicorn/consistent-function-scoping).
...
describe("injectFeatures: 3D buildings", () => {
    ...
    it("is idempotent across re-application", () => {
        ...
        expect(twice.layers.filter((l) => l.id === "femto-3d-buildings")).toHaveLength(1);
    });
...
describe("injectFeatures: accent recolour", () => {
    ...
    it("recolours roads by class: casing, minor (incl. path/subtle), major", () => {
        const style = injectFeatures(baseStyle(), { ...OFF, accent });
        expect(paintOf(style, "highway_motorway_casing")?.["line-color"]).toBe(accent.roadCasing);
        expect(paintOf(style, "tunnel_motorway_casing")?.["line-color"]).toBe(accent.roadCasing);
        expect(paintOf(style, "highway_minor")?.["line-color"]).toBe(accent.roadMinor);
        expect(paintOf(style, "highway_path")?.["line-color"]).toBe(accent.roadMinor);
        expect(paintOf(style, "highway_major_subtle")?.["line-color"]).toBe(accent.roadMinor);
        expect(paintOf(style, "highway_motorway_inner")?.["line-color"]).toBe(accent.roadMajor);
    });
