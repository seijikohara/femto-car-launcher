...
// .oxlintrc.json and no-let.js). All camera pushes and API ops read+write through here.
const state = {
...
async function initMap(): Promise<void> {
    ...
    const liveMap = new mapsLib.Map(mapEl as HTMLElement, {
        ...
        ...(isVector ? { mapId, heading: 0, tilt: 0, headingInteractionEnabled: false } : {}),
    });
    state.map = liveMap;
    ...
    if (pending.options) window.setGoogleMapsOptions(pending.options[0], pending.options[1]);
