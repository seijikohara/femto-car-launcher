...
// above still holds.
function report(kind: "fatal" | "error" | "follow" | "bearing", detail: unknown): void {
    try {
...
// Mutable page state in one const holder (let/var are banned — see
// .oxlintrc.json and no-let.js). Camera pushes, style swaps, and error throttling all read
...
try {
    ...
    setInterval(() => {
        const stale =
            state.lastFixMs > 0 && Date.now() - state.lastFixMs > LOCATION_STALE_THRESHOLD_MS;
    ...
    function easeHome(durationMs: number): void {
        ...
        liveMap.easeTo({
            ...
            padding: {
                ...
                bottom: 0,
                left: markerPadLeft(fix.leftSafe, liveMap.getContainer().clientWidth || 0),
                right: markerPadRight(fix.rightSafe, liveMap.getContainer().clientWidth || 0),
            },
            duration: durationMs,
            essential: true,
        });
    ...
    window.updateCamera = (
        ...
        const opts = {
            ...
            pitch: tilt || 0,
            padding: {
                top: markerPadTop(markerPos, bottomSafe, liveMap.getContainer().clientHeight || 0),
                bottom: 0,
                left: markerPadLeft(leftSafe, liveMap.getContainer().clientWidth || 0),
                right: markerPadRight(rightSafe, liveMap.getContainer().clientWidth || 0),
            },
        };
