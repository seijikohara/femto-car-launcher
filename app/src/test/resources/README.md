...
## `map-backdrop-osm.png` — still map for the dashboard goldens
...
**Provenance.** A device capture of this app rendering the OSM backend
(OpenFreeMap vector tiles, OpenStreetMap data under ODbL) over San Francisco,
cropped clear of every UI overlay. The dashboard fixtures place the fix and the
reverse-geocoded address in the same city so the two agree.

Recapture only when the map's own appearance changes (a style or backend
default): run the app on the OSM backend, screenshot a frame with no overlay in
the crop area, and crop to a large landscape-ish rectangle. It is scaled with
