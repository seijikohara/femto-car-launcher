...
internal class SettingsFactsCollector(
    ...
    suspend fun settingsFacts(): SectionPayload.Facts {
        ...
        return SectionPayload.Facts(displayFacts(display) + locationFacts(location))
    }

    private fun displayFacts(display: DisplaySettings): List<DiagnosticFact> =
        listOf(
    ...
    private fun locationFacts(location: LocationSettings): List<DiagnosticFact> =
        listOf(
