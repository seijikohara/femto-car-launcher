...
// Single source for the map-recolor rule data: webmap/src/map-recolor-data.json is
// imported by the live backend (webmap/src/style.ts) and code-generated into a
// Kotlin object here for the snapshot backend, so the accent layer groups and the
// marker-drop cap live in one file instead of literals mirrored on both sides.
val mapRecolorJson = rootProject.file("webmap/src/map-recolor-data.json")
val mapRecolorGenDir = layout.buildDirectory.dir("generated/source/maprecolor")
val generateMapRecolorData =
    tasks.register("generateMapRecolorData") {
        inputs.file(mapRecolorJson)
        outputs.dir(mapRecolorGenDir)
        doLast {
            @Suppress("UNCHECKED_CAST")
            val data = groovy.json.JsonSlurper().parse(mapRecolorJson) as Map<String, Any>

            fun quotedList(key: String) = (data.getValue(key) as List<String>).joinToString(", ") { "\"$it\"" }
            mapRecolorGenDir
                .get()
                .dir("io/github/seijikohara/femto/ui/home/components")
                .asFile
                .apply { mkdirs() }
                .resolve("MapRecolorData.kt")
                .writeText(
                    """
                    |// GENERATED by the generateMapRecolorData Gradle task from
                    |// webmap/src/map-recolor-data.json — do not edit by hand. That JSON is the
                    |// single source the live backend (webmap/src/style.ts) imports too.
                    |package io.github.seijikohara.femto.ui.home.components
                    |
                    |internal object MapRecolorData {
                    |    val accentLandLayers: Set<String> = setOf(${quotedList("accentLandLayers")})
                    |    val roadMinorKeywords: List<String> = listOf(${quotedList("roadMinorKeywords")})
                    |    const val maxMarkerDrop: Double = ${data.getValue("maxMarkerDrop")}
                    |}
                    """.trimMargin() + "\n",
                )
        }
...
// The SourceSet API takes a path, not a Provider; the generateMapRecolorData
// dependency below guarantees the directory is populated before compilation.
android.sourceSets
    .getByName("main")
    .kotlin
    .srcDir(mapRecolorGenDir.get().asFile)
tasks.named("preBuild") { dependsOn(generateMapRecolorData) }
tasks.withType<org.jetbrains.kotlin.gradle.tasks.KotlinCompile>().configureEach {
    dependsOn(generateMapRecolorData)
}
